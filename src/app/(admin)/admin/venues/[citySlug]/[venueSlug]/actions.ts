...
function adminVenuePath(citySlug: string, venueSlug: string) {
  return `/admin/venues/${citySlug}/${venueSlug}`;
}
...
export async function updateVenueDetails(formData: FormData) {
  ...
  const venueCitySlug = getText(formData, "city_slug");
  ...
  revalidatePath(adminVenuePath(venueCitySlug, venueSlug));
}
...
export async function uploadVenuePhoto(formData: FormData) {
  ...
  const venueCitySlug = getText(formData, "venue_city_slug");

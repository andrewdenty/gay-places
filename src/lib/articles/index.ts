...
function parseArticle(filename: string): ArticleMeta | null {
  ...
  return {
    ...
    author: data.author ?? "",
    publishedAt: data.publishedAt instanceof Date ? data.publishedAt.toISOString() : (data.publishedAt?.toString() ?? ""),
    updatedAt: data.updatedAt instanceof Date ? data.updatedAt.toISOString() : data.updatedAt?.toString(),
